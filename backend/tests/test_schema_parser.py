...
Feature count updated to 49 (4 CSIC-bias features dropped — see scripts/retrain_49.py).
"""
...
class TestSchemaParser:
    ...
    def test_parse_valid_schema(self):
        ...
        assert schema.total_features == EXPECTED_FEATURES
        assert len(schema.features) == EXPECTED_FEATURES
    ...
    def test_validate_feature_count(self):
        """Test validation of exactly 49 features (Requirement 12.3)."""
    ...
    def test_validate_feature_count_mismatch(self):
        ...
        invalid_data = {
            ...
            "features": ["url_feature_" + str(i) for i in range(50)],
            "srs_requirements": [],
            "data_sources": {}
        }
        ...
        assert "exactly 49 features" in str(exc_info.value)
    ...
    def test_validate_invalid_feature_name(self):
        ...
        invalid_data = {
            ...
            "feature_groups": {"test": EXPECTED_FEATURES},
            "features": ["InvalidFeature"] + ["url_feature_" + str(i) for i in range(EXPECTED_FEATURES - 1)],
            "srs_requirements": [],
            "data_sources": {}
        }
        ...
        assert (
            "does not match expected pattern" in str(exc_info.value)
            or "does not start with expected prefix" in str(exc_info.value)
        )
    ...
    def test_to_json_pretty_printer(self):
        ...
        assert parsed["total_features"] == EXPECTED_FEATURES
        assert len(parsed["features"]) == EXPECTED_FEATURES
    ...
    def test_validate_features_wrong_count(self):
        ...
        assert f"Expected {EXPECTED_FEATURES} features" in str(exc_info.value)

    def test_validate_features_wrong_names(self):
        """Test validate_features fails with wrong feature names."""
        schema = load_schema("FEATURE_SCHEMA.json")

        features = {f"wrong_feature_{i}": 0.0 for i in range(EXPECTED_FEATURES)}
...
class TestFeatureSchema:
    ...
    def test_dropped_features_absent(self):
        """Verify the 4 CSIC-bias features are not in the schema."""
        schema = load_schema("FEATURE_SCHEMA.json")

        dropped = ["cookie_is_present", "cookie_length",
                   "connection_is_close", "connection_keep_alive"]
        for feat in dropped:
            assert feat not in schema.features, \
                f"Dropped feature '{feat}' should not be in schema"
